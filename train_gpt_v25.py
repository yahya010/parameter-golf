...
_USE_FA3 = bool(int(os.environ.get("USE_FA3", "1")))
...
def _sdpa_attn(q, k, v, causal=True):
    """SDPA fallback when flash_attn_3 is not available."""
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
...
def _attn_func(q, k, v, causal=True):
    if _USE_FA3 and flash_attn_3_func is not None:
...
class Hyperparameters:
    ...
    slot_stride = int(os.environ.get("SLOT_STRIDE", 0))  # 0 = use eval_stride
...
def eval_val(
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            _dev_type = "cuda" if device.type == "cuda" else device.type
            with torch.autocast(device_type=_dev_type, dtype=torch.bfloat16, enabled=(device.type == "cuda")):
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        y = _attn_func(q, k, v, causal=True)
        if self.use_xsa:
...
class GPT(nn.Module):
    ...
    def forward_hidden(self, input_ids: Tensor) -> Tensor:
        """Return final hidden states [bsz, seq_len, dim] before lm_head."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
...
def eval_val_sliding(
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            with torch.autocast(device_type=_dev.type if _dev.type == "cuda" else "cpu", dtype=torch.bfloat16, enabled=(_dev.type == "cuda")):
...
def eval_val_slot(
    ...
    """SLOT: per-window delta + logit_bias optimization with frozen model weights.
    Based on arXiv:2505.12392v2, PRs #1176/#1229/#1313."""
    seq_len = eval_seq_len or args.train_seq_len
    slot_steps = args.slot_steps
    slot_lr = args.slot_lr
    slot_lr_min = args.slot_lr_min
    slot_wd = args.slot_wd
    vocab_size = args.vocab_size
    model_dim = args.model_dim
    softcap = args.logit_softcap
    tie_emb = args.tie_embeddings
    ...
    eval_model.eval()
    if tie_emb:
        lm_weight = eval_model.tok_emb.weight.detach()
    else:
        lm_weight = eval_model.lm_head.weight.detach()
    ...
    for bi in range(0, len(my_windows), batch_seqs):
        ...
        y_batch = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
        wlens: list[int] = []
        score_masks = torch.zeros(bsz, seq_len, dtype=torch.bool, device=device)
        ...
        for i, ws in enumerate(batch_ws):
            ...
            y_batch[i, :wlen] = chunk[1:]
            s = 0 if ws == 0 else max(wlen - stride, 0)
            score_masks[i, s:wlen] = True

        with torch.no_grad():
            amp_dtype = torch.bfloat16
            dev_type = "cuda" if device.type == "cuda" else device.type
            with torch.autocast(device_type=dev_type, dtype=amp_dtype, enabled=(dev_type == "cuda")):
                hidden = eval_model.forward_hidden(x_batch)
        ...
        opt = torch.optim.AdamW([delta, logit_bias], lr=slot_lr, weight_decay=slot_wd)

        for step_i in range(slot_steps):
            frac = step_i / max(slot_steps - 1, 1)
            cur_lr = slot_lr_min + 0.5 * (slot_lr - slot_lr_min) * (1 + math.cos(math.pi * frac))
            for pg in opt.param_groups:
                pg["lr"] = cur_lr

            opt.zero_grad()
            h = hidden + delta
            logits_proj = F.linear(h, lm_weight.float()) + logit_bias
            ...
            masked_loss = (per_token_nll * score_masks.float()).sum() / score_masks.float().sum().clamp_min(1.0)
            masked_loss.backward()
            opt.step()

        with torch.no_grad():
            h = hidden + delta.detach()
            logits_proj = F.linear(h, lm_weight.float()) + logit_bias.detach()
            logits = softcap * torch.tanh(logits_proj / softcap)
            nll = F.cross_entropy(
                logits.reshape(-1, vocab_size),
                y_batch.reshape(-1),
...
def main() -> None:
    ...
    if args.slot_enabled:
        slot_stride = args.slot_stride if args.slot_stride > 0 else args.eval_stride
        if device.type == "cuda":
            _sync()
        ...
        log0(
            ...
            f"steps:{args.slot_steps} lr:{args.slot_lr} stride:{slot_stride} "
